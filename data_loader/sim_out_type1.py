...
__all__ = ['sim_out_type1']


class sim_out_type1(data.Dataset):
    BASE_DIR = 'sim_out_1'
    NUM_CLASS = 6 #classification type num, [ex) grass, chair = 2]

    def __init__(self, root='./datasets', split='train', mode=None, transform=None,
                 base_size=520, crop_size=480, **kwargs):
        super(sim_out_type1, self).__init__()
        self.root = root # file path
        self.split = split # file path
        ...
        self._key = np.array([0, 1, 2, 3, 4, 5])
        
        self._mapping = np.array(range(len(self._key))).astype('int32')
...
def _get_city_pairs(folder, split='train'):
        ...
        for root, _, files in os.walk(img_folder):
            for filename in files:
                if filename.endswith(".jpg") and '_mask' not in filename:
                    ...
                    maskname = filename.replace('.jpg', '_mask.png')
    ...
    if split in ('train', 'valid'):
        ...
        img_paths, mask_paths = get_path_pairs(img_folder)
        return img_paths, mask_paths
    else:
        assert split == 'trainval'
        print('trainval set')
        train_img_folder = os.path.join(folder, 'train')
        val_img_folder = os.path.join(folder, 'valid')
