...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='sim_out_1',
                        help='dataset name (default: sim_out_1)') ## data_set type
...
class Trainer(object):
    def __init__(self, args):
        ...
        root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), 'data_loader/datasets'))
        data_kwargs = {'root': root_dir, 'transform': input_transform, 'base_size': args.base_size, 'crop_size': args.crop_size}
        print(f"Loading training dataset: {args.dataset} with split: {args.train_split}")
        
        train_dataset = get_segmentation_dataset(args.dataset, split=args.train_split, mode='train', **data_kwargs)
        print(f"Training dataset root: {train_dataset.root}")
        print(f"Found {len(train_dataset.images)} images for training")
        
        val_dataset = get_segmentation_dataset(args.dataset, split='valid', mode='valid', **data_kwargs)
        print(f"Validation dataset root: {val_dataset.root}")
        print(f"Found {len(val_dataset.images)} images for validation")
